...
#| echo: FALSE
# #Create a design matrix
design_c <- model.matrix(~ -1 + colony + WBC_ratio_log, data = d_n_complete$samples) #model contrasts without intercept is easier
...
#
# #voom() converts the read counts to log2-cpm, with associated weights, ready for linear modelling
vobj_c <- voom(d_n_complete, design_c, plot=TRUE)
#
# # Fit a random effect
dupcor_c <- duplicateCorrelation(vobj_c, design_c, block = d_n_complete$samples$ID)
dupcor_c$consensus #the estimated intra individual correlation (0.1902523)
# #the intra individual correlation will change the voom weights slightly
# #run voom again considering the duplicateCorrelation results in order to compute more accurate precision weights
vobj_c = voom(d_n_complete, design_c, plot=TRUE, 
          block=d_n_complete$samples$ID, correlation=dupcor_c$consensus)
# #update the correlation for the new voom weights
dupcor_c <- duplicateCorrelation(vobj_c, design_c, block = d_n_complete$samples$ID)
...
# #The model forces the magnitude of the random effect to be the same across all genes.
# #Estimate linear mixed model with a single variance component
vfit_c <- lmFit(vobj_c, design_c, block=d_n_complete$samples$ID, correlation=dupcor_c$consensus)
...
# #Create a design matrix
design_y <- model.matrix(~ -1 + year + WBC_ratio_log, data = d_n_complete$samples) #model contrasts without intercept is easier
colnames(design_y) <- c("Season1", "Season2", "WBC_ratio_log") #clean group names
# #create matrix for contrasts
...
#
# #voom() converts the read counts to log2-cpm, with associated weights, ready for linear modelling
vobj_y <- voom(d_n_complete, design_y, plot=TRUE)
# 
# #Fit a random effect
dupcor_y <- duplicateCorrelation(vobj_y, design_y, block = d_n_complete$samples$ID)
dupcor_y$consensus #the estimated intra individual correlation (0.1630921)
# #the intra individual correlation will change the voom weights slightly
# #run voom again considering the duplicateCorrelation results in order to compute more accurate precision weights
vobj_y = voom(d_n_complete, design_y, plot=TRUE, 
            block=d_n_complete$samples$ID, correlation=dupcor_y$consensus)
# #update the correlation for the new voom weights
dupcor_y <- duplicateCorrelation(vobj_y, design_y, block = d_n_complete$samples$ID)
dupcor_y$consensus #the estimated intra individual correlation (0.1631328)
#
# #Analyzing repeated measures data using duplicateCorrelation.
# #The model forces the magnitude of the random effect to be the same across all genes.
#
# #Estimate linear mixed model with a single variance component
vfit_y <- lmFit(vobj_y, design_y, block=d_n_complete$samples$ID, correlation=dupcor_y$consensus)
...
ggsave(here::here("Plots","Bubble_LHSTP.png"), plot = Bubble_together, width = 4, height = 10, dpi = 600)  
```
...
![Heatmap: GOterms (Time Point)](Plots/HP_GO_TP.png) \### Life history stage
