...
tp_plot <- ggplot(Allgenes_tp, aes(x = logFC, y = -log10(P.Value), color = Significance, size = Significance)) +
  ...
  labs(title = "Time point (End VS Start)", x = "Log2 fold change", y = "-Log10 P-value") +
  theme_classic() +
  ylim(0,60) +
  xlim(-4,4) +
...
tfit_lhs <- treat(vfit_lhs, lfc=lfc_half) 
topTreat(tfit_lhs)
nrow(tfit_lhs)
...
Filteredgenes_lhs <- subset(Allgenes_lhs, abs(logFC) >= lfc_half & round(adj.P.Val, 2) < adj.P.value)
Upregulated_lhs <-subset(Filteredgenes_lhs, Filteredgenes_lhs$logFC >= lfc_half)
Downregulated_lhs <-subset(Filteredgenes_lhs, Filteredgenes_lhs$logFC <= lfc_half)
...
Allgenes_lhs$Significance[Allgenes_lhs$adj.P.Val < adj.P.value & Allgenes_lhs$logFC >= lfc_half] <- "Upregulated"
Allgenes_lhs$Significance[Allgenes_lhs$adj.P.Val < adj.P.value & Allgenes_lhs$logFC <= lfc_half] <- "Downregulated"
...
lhs_plot <- ggplot(Allgenes_lhs, aes(x = logFC, y = -log10(P.Value), color = Significance, size = Significance)) +
  ...
  scale_color_manual(values = clrs) +  
  scale_size_manual(values = c(1.5, 0.5, 1.5)) +
  labs(title = "Life history stage (Adult VS Pup)", x = "Log2 fold change", y = "-Log10 P-value") +
  theme_classic() +
  #ylim(0,60) +
  #xlim(-4,4) +
  theme(legend.position = "top")
...
c_plot <- ggplot(Allgenes_c, aes(x = logFC, y = -log10(P.Value), color = Significance, size = Significance)) +
  ...
  labs(title = "Colony (SSB VS FWB)", x = "Log2 fold change", y = "-Log10 P-value") +
  theme_classic() +
  ylim(0,60) +
  xlim(-4,4) +
  theme(legend.position = "top")
...
y_plot <- ggplot(Allgenes_y, aes(x = logFC, y = -log10(P.Value), color = Significance, size = Significance)) +
  ...
  scale_color_manual(values = clrs) +  
  scale_size_manual(values = 0.5) +
  labs(title = "Season (2018-2019 VS 2019-2020)", x = "Log2 fold change", y = "-Log10 P-value") +
  theme_classic() +
  ylim(0,60) +
  xlim(-4,4) +
  theme(legend.position = "top")
...
tfit_interaction3 <- treat(vfit_interaction3, lfc=lfc_half) 
...
#Filtered
Filtered_EndvsStartMum <- subset(Allgenes_EndvsStartMum, abs(logFC) >= lfc_half & adj.P.Val < adj.P.value)
Filtered_EndvsStartPup <- subset(Allgenes_EndvsStartPup, abs(logFC) >= lfc_half & adj.P.Val < adj.P.value)
Filtered_Interaction3 <- subset(Allgenes_Interaction3, abs(logFC) >= lfc_half & adj.P.Val < adj.P.value)
...
cont.matrix.interaction4 <- makeContrasts(
  ...
  FWBLHS = FWB.Mum - FWB.Pup, #which genes in FWB are DE among lhs
  SSBLHS = SSB.Mum - SSB.Pup, #which genes in SSB are DE among lhs
...
tfit_interaction4 <- treat(vfit_interaction4, lfc=lfc_half) 
