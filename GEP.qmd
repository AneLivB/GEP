...
#ggsave(here::here("Plots","heatmap_tp.png"), plot = heatmap_tp, width = 7, height = 7, dpi = 600)

png("heatmap_tp.png", width=6, height=8, units="in", res=600)
pheatmap(matrix_tp,
         scale = "row",
         color = colorRampPalette(c("#9D00FF", "white", "#FFA500"))(100),
         annotation_colors = ann_colors,
...
plot_LHS <- ggplot(GO_LHS, aes(x = zScore, y = -log10(p.adjust))) +
  ...
  labs(
    title = "Enrichment analysis (Life history stage)",
    x = "Z-score",
    y = expression(-log[10]("Adjusted p-value (FDR)")),
    shape = "Regulation"
  ) +
...
#| echo: FALSE
SP_GO_TP_UP <- scatterPlot(SM_GO_TP_UP, reduced_TP_UP) + 
  theme_classic() + 
  xlim(c(-1,1)) + 
  ylim(c(-1,1)) +
...
SP_GO_TP_DOWN <- scatterPlot(SM_GO_TP_DOWN, reduced_TP_DOWN) + 
  theme_classic() + 
  xlim(c(-1,1)) + 
  ylim(c(-1,1)) +
  xlab("Semantic space x") +
...
SP_GO_LHS_UP <- scatterPlot(SM_GO_LHS_UP, reduced_LHS_UP) + 
  theme_classic() + 
  xlim(c(-1,1)) + 
  ylim(c(-1,1)) +
  xlab("Semantic space x") +
...
SP_GO_LHS_DOWN <- scatterPlot(SM_GO_LHS_DOWN, reduced_LHS_DOWN) + 
  theme_classic() +
  xlab("Semantic space x") +
  ylab("Semantic space y") +
