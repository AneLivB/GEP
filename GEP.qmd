...
```{r}
clrs <- c("cornflowerblue", "grey20", "plum3")
```
...
# As ggplot 
# Extract the data 
tp_data <- data.frame(AveExpr = tfit$Amean,  
  logFC = tfit$coefficients[,1], 
  Status = factor(dt[,1]))

tp_plot <- ggplot(tp_data, aes(x = AveExpr, y = logFC, color = Status, size = Status)) +
  geom_point() +
...
# As ggplot 
# Extract the data 
lhs_data <- data.frame(AveExpr = tfit_lhs$Amean,  
  logFC = tfit_lhs$coefficients[,1], 
  Status = factor(dt_lhs[,1]))

lhs_plot <- ggplot(lhs_data, aes(x = AveExpr, y = logFC, color = Status, size = Status)) +
  geom_point() +
  scale_color_manual(values = clrs) + 
  scale_size_manual(values = c(1.5, 0.5, 1.5)) +
  labs(title = "Adults vs Pups", x = "Mean Expression (logCPM)", y = "Log Fold Change") +
  theme_classic() +
  theme(legend.position = "top")
```
## Colony
...
```{r}
d_n$samples$colony <- as.factor(d_n$samples$colony) #colony as factor
#Create a design matrix
...
#set log-fold-changes (log-FCs) to be above a minimum value. The treat method (McCarthy and Smyth 2009) used to calculate p-values from empirical Bayes moderated t-statistics with a minimum log-FC requirement. 
#no need to use the ebayes output for this
lfc_c <- 1 #log fold change of 1 is pretty standard but explore further
#a doubling in the original scaling is equal to a log2 fold change of 1
#a quadrupling is equal to a log2 fold change of 2
#Positive values means the gene is more expressed
tfit_c <- treat(vfit_c, lfc=lfc_c) 
topTreat(tfit_c)
nrow(tfit_c)
dt_c <- decideTests(tfit_c, adjust.method="fdr")
summary(dt_c) 
#plotMD(tfit_c, column=1, status=dt_c[,1], main=colnames(tfit_c)[1])
# As ggplot 
# Extract the data 
c_data <- data.frame(AveExpr = tfit_c$Amean,  
  logFC = tfit_c$coefficients[,1], 
  Status = factor(dt_c[,1]))

c_plot <- ggplot(c_data, aes(x = AveExpr, y = logFC, color = Status, size = Status)) +
  geom_point() +
...
```{r}
d_n$samples$year <- as.factor(d_n$samples$year) #year as factor
#Create a design matrix
...
#set log-fold-changes (log-FCs) to be above a minimum value. The treat method (McCarthy and Smyth 2009) used to calculate p-values from empirical Bayes moderated t-statistics with a minimum log-FC requirement. 
#no need to use the ebayes output for this
lfc_y <- 1 #log fold change of 1 is pretty standard but explore further
#a doubling in the original scaling is equal to a log2 fold change of 1
#a quadrupling is equal to a log2 fold change of 2
#Positive values means the gene is more expressed
tfit_y <- treat(vfit_y, lfc=lfc_y) 
topTreat(tfit_y)
nrow(tfit_y)
dt_y <- decideTests(tfit_y, adjust.method="fdr")
summary(dt_y) 
#plotMD(tfit_y, column=1, status=dt_y[,1], main=colnames(tfit_y)[1])
# As ggplot 
# Extract the data 
y_data <- data.frame(AveExpr = tfit_y$Amean,  
  logFC = tfit_y$coefficients[,1], 
  Status = factor(dt_y[,1]))

y_plot <- ggplot(y_data, aes(x = AveExpr, y = logFC, color = Status, size = Status)) +
  geom_point() +
  scale_color_manual(values = "grey20") +  
  scale_size_manual(values = 0.5) +
  labs(title = "2018 vs 2019", x = "Mean Expression (logCPM)", y = "Log Fold Change") +
  theme_classic() +
  theme(legend.position = "top")
...
plot_grid(y_plot, lhs_plot, c_plot, tp_plot)
```
