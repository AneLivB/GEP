...
legend_theme <- theme(
    legend.position = c(0.95, 0.95),  # (x, y) coordinates from bottom-left = (0,0) to top-right = (1,1)
    legend.justification = c("right", "top"),  # Anchor legend by its top-right corner
    legend.background = element_rect(fill = alpha('white', 0.7), color = NA),  # Optional: translucent background
    legend.key.size = unit(0.8, "lines")  # Optional: adjust size
  )

PCA_plots <- ggarrange(PCA_TP + legend_theme, PCA_LHS + legend_theme, 
                       PCA_Y + legend_theme, PCA_C + legend_theme)
