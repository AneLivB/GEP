...
#| echo: FALSE
#here::here()
invisible(pacman::p_load(dplyr, readxl, tidyverse, cowplot, edgeR, DESeq2, RColorBrewer, ggplot2, reshape2, ggpubr))
...
![Principle component analysis of raw data](Plots/PCA_plots.png)
...
```{r}
#Bubble plot fancy
BB_TP <- ggplot(rbind(as.data.frame(GO_results_list$TP_DOWN), as.data.frame(GO_results_list$TP_UP)), aes(x = zScore, y = -log10(p.adjust))) +
  geom_point(aes(size = Count, fill = Description), shape = 24, color = "black", alpha = 0.7) +
  scale_size_continuous(name = "DEG Count") +
  labs(
    title = "Significantly Enriched GO Terms for DEGs (Time point)",
    x = "Z-score",
    y = expression(-log[10]("Adjusted p-value (FDR)"))
  ) +
  theme_classic() +
...
#Bubble plot fancy
BB_LHS <- ggplot(rbind(as.data.frame(GO_results_list$LHS_DOWN), as.data.frame(GO_results_list$LHS_UP)), aes(x = zScore, y = -log10(p.adjust))) +
  geom_point(aes(size = Count, fill = Description), shape = 24, color = "black", alpha = 0.7) +
  scale_size_continuous(name = "DEG Count") +
  labs(
    title = "Significantly Enriched GO Terms for DEGs (Life history stage)",
    x = "Z-score",
    y = expression(-log[10]("Adjusted p-value (FDR)"))
  ) +
  theme_classic() +
...
plot_grid(BB_TP, BB_LHS)

ggplot(top20_labels_TP, aes(x = Regulation, y = Description)) +
  geom_point(aes(size = Count, color = Regulation, fill = Regulation), shape = 21) +
  scale_size_continuous(name = "Gene Count") +
  scale_fill_manual(values = c("Upregulated" = "#1f77b4", "Downregulated" = "#d62728")) +
  scale_color_manual(values = c("Upregulated" = "#1f77b4", "Downregulated" = "#d62728")) +
  labs(
    title = "Top 20 Enriched GO Biological Processes (Up & Downregulated)",
    x = "Regulation",
    y = "GO Term"
  ) +
  theme_classic() +
...
barplot(GO_results_list$TP_DOWN, showCategory = 20) +
  ggtitle("Top Enriched GO Biological Processes")

top20_labels_LHS <- rbind(as.data.frame(GO_results_list$LHS_DOWN), as.data.frame(GO_results_list$LHS_UP)) %>%
  arrange(p.adjust) %>%
  slice_head(n = 20)

barplot(top20_labels_LHS, showCategory = 20) +
  ggtitle("Top Enriched GO Biological Processes")
```
...
sim_matrices <- setNames(
  lapply(GO_names, function(name) {
    ...
    if (!is.null(df) && nrow(df) > 1) {
      ...
      NULL  # skip or handle cases where df is NULL or too small
    }
  }),
  GO_names
)
