...
PCA_sex <- ggplot(pc_scores_pups, aes(x = PC1_GE, y = PC2_GE, color = sex, shape = sex)) +
  ...
  xlim(-200, 200) +
  ylim(-200, 200) +
  labs(title = "Offspring sex", x = "PC1 (18 %)", y = "PC2 (15 %)", color = NULL, shape = NULL)
...
#PERMANOVA with contrasts
adonis2(distance_matrix_pups ~ group + year + colony + sex,
        data = d_n_pups_all$samples, permutations = 5000, by = "margin") #no effect of sex

#PERMANOVA biological
adonis2(distance_matrix_pups ~ sex + WBC_ratio_log + PC1 + PC2 + PC3 + PC4 + PC5,
        data = d_n_pups_all$samples, permutations = 5000, by = "margin") #no effect of sex

#PERMANOVA full
adonis2(distance_matrix_pups ~ group + year + colony + sex + WBC_ratio_log + PC1 + PC2 + PC3 + PC4 + PC5,
        data = d_n_pups_all$samples, permutations = 5000, by = "margin") #no effect of sex
...
design_sex <- model.matrix(~ 0 + sex + group + year + colony + WBC_ratio_log + PC1 + PC2 + PC3 + PC4 + PC5, data = d_n_pups_all$samples) 
